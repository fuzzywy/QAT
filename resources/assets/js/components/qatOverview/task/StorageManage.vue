            ...
            >
  			...
  			<el-card class="box-card" style="height: 100%;">
			  	<div slot="header" class="clearfix">
			    	...
			    	<el-button size="small" type="primary" style="float:right;margin-left: 10px;" @click="stopTask">{{$t('messages.common.stop')}}</el-button>
			    	<el-button size="small" type="primary" style="float:right" @click="runTask" :runTaskData="runTaskData">{{$t('messages.common.run')}}</el-button>
			    	<el-button size="small" type="primary" style="float:right" @click="deleteTask" :loading="loading.deleteTaskStatus">{{$t('messages.common.delete')}}</el-button>
...
<script>
	export default {
		...
		methods : {
            ...
            saveTaskData () {
            	switch( this.$store.getters.saveTaskDataStatus ) {
                    case 2:
                    	...
                    	if (this.$store.getters.saveTaskData)
	                    	this.$message.error({
		    ...
		    deleteTask() {
		    	var _this = this;
		    	if( !(this.currentRow && this.currentRow.taskName)){
                    this.$message({
                        type: 'warning',
                ...
                if(this.$store.getters.getUserData.type != 'admin' && this.$store.getters.getUserData.name != this.currentRow.owner){
                    this.$message.warning(this.$t('messages.task.operateTip'));
                    return false;
                }
		    	...
		    	this.$confirm(this.$t('messages.task.deleteConfirm')+'：'+this.currentRow.taskName+'?', this.$t('messages.common.tip'), {
		          	...
		          	this.$store.dispatch( 'deleteTaskData',this.currentRow).then(function(){
