...
{   
    ...
    */
    public function getMoByParamFilter() {
        ...
        $moTreeData = [];
        Config::set('database.connections.kget.database', $this->dbName);
        $dataFilterByParam = COLUMNS::selectRaw('distinct TABLE_NAME')->where('TABLE_SCHEMA','=',$this->dbName)->where('COLUMN_NAME','like','%'.$filterText.'%');
        $data = TABLES::select('TABLE_NAME')->where('TABLE_SCHEMA','=',$this->dbName)->where('TABLE_NAME','like','%'.$filterText.'%')->union($dataFilterByParam)->get()->toArray();
        for ($i=0; $i < count($data); $i++) { 
            $moTreeData[] = array('id'=>$i,'label'=>$data[$i]['TABLE_NAME']);
        }
        return json_encode($moTreeData);
    }
    /**  
    ...
    */
    public function getKgetTask() {
    	...
    	$tasks     = TABLES::select('TABLE_SCHEMA')->where('TABLE_NAME','=','moTreeInfo')->orderBy('TABLE_SCHEMA', 'desc')->get()->toArray();
        $kgetTasks = array();
        foreach ($tasks as $task) {
            $kgetTasks[] = array("value"=>$task['TABLE_SCHEMA'],"label"=>$task['TABLE_SCHEMA']);
        }
    ...
    */
    public function getKgetData() {
        ...
        $result         = array();
        $totalQuery     = DB::connection('kget')->table($moName);
        ...
        if ($city) {
        	...
        	$totalQuery  = $totalQuery->whereIn('subNetwork',$subNetGroup);
        ...
        if ($baseStation) {
        	...
        	$totalQuery       = $totalQuery->whereIn('meContext',$baseStationGroup);
    ...
    */
    public function exportKgetFile() {
        ...
        $sql      = $title.$sql." INTO OUTFILE '".$fileName."'
