    ...
    public function getLoadTabs() {
        ...
        if($city!="全省"&&$city!="province"){
            ...
            $res = B_L_GSM_Day::where("location",$city)
                            ->where("day_id",function($query)use($city){
                                $query->select('day_id')->from("B_L_GSM_Day")->where("location",$city)->orderBy("day_id","desc")->limit(1);
                            })->get()->toArray();
            ...
            // print_r($day_id[0]['day_id']);
             ...
             $res = B_L_TDD::where("location",$city)
                            ->where("day_id",function($query)use($city){
                                $query->select('day_id')->from("B_L_TDD")->where("location",$city)->orderBy("day_id","desc")->limit(1);
                            })->get()->toArray();
            $key = array_keys($res[0]);
            $len = count($key);
            ...
            $res = B_L_FDD::where("location",$city)
                            ->where("day_id",function($query)use($city){
                                $query->select('day_id')->from("B_L_FDD")->where("location",$city)->orderBy("day_id","desc")->limit(1);
                            })->get()->toArray();
                $key = array_keys($res[0]);
                $len = count($key);
             ...
             $res = B_L_NBIOT::select()->where("location",$city)
                                       ->where("day_id",function($query)use($city){
                                $query->select('day_id')->from("B_L_NBIOT")->where("location",$city)->orderBy("day_id","desc")->limit(1);
                                })->get()->toArray();
             // print_r($res);exit;
            $key = array_keys($res[0]);
            $len = count($key);
